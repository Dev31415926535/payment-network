...
async function getOnChainData() {
  const account = await client.getAccount("0xUserWalletAddress");
...
const borrowed = mongoose.model('borrowed',{
  ...
  ownerName: {type: String,required: true },
  ownerEmail: {type: String,required: true },
  lenderNmae: {type: String,required: true},
...
app.post('/items', async (req, res) => {
  try {
      const { name , description, username, contact } = req.body;
...
app.get('/getitems', async (req, res) => {
  try {
    ...
    const records = await items.find();  // Item is the model you've defined
    console.log(records);
